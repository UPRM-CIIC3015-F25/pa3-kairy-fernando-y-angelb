...
class DeckManager:
    ...
    def createDeck(self, subLevel: SubLevel = None):
        ...
        for suit in suits: #LOOP PARA RECORRER TO'
            for rank in ranks:
                image = cardImages.get((suit, rank))
                if image is None:
                    continue  # basicamente skipea para que no haya algun error

                deck.append(Card(suit=suit, rank=rank, image=image))

        return deck

    # DONE - kairy (TASK 5.1): Complete the priceMap variable by assigning each joker a price.
