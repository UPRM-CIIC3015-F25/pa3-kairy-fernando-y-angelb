...
class GameState(State):
    ...
    def calculate_gold_reward(self, playerInfo, stage=0):
        ...
        raw_bonus = ((score - target) / target) * 5
        clamped_bonus = max(0, raw_bonus)
        bonus = min(5, clamped_bonus)
    ...
    def discardCards(self, removeFromHand: bool):
        ...
        if len(self.cardsSelectedList)==0:
            #refill a la mano
            falta=8-len(self.hand)
            if falta > 0:
                newcards=State.deckManager.dealCards(self.deck,falta,self.playerInfo.levelManager.curSubLevel)
                for c in newcards:
                    self.hand.append(c)
            #RESETIAO TO
